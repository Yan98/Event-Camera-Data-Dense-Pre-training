...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr  2 17:05:00 2023
...
def viz_events(events, resolution):
    ...
    neg_events = events[events[:,-1]<0.5]
    neg_events[:,-1] = -1
    ...
    #print(np.max(image_pos),np.max(image_neg))
    image_rgb = np.stack(
        [
    ...
    return image_rgb + ((image_rgb.astype(np.float32).sum(-1,keepdims=True) < 0.001) * 255).astype(np.uint8)

p = '/home/yan181/Downloads/TartanAir/V2E_DataSim/000008_000009_event.h5'
#p = '/home/yan181/Downloads/TartanAir/EMA_DataSim/000015_000016_event.hdf5' 
event = read_event_h5(p)

#event = event[event[:,2] > 15 * 1e6]
#event = eventsToVoxel(event,29,480,640,True).reshape(29,2,480,640)
...
for i in range(1,SPLIT+1):
    mask = np.logical_and(event[:,2] < d/SPLIT * i, event[:,2] >  d/SPLIT * max(i-1,0))
    e = event[mask]
    print(i, e.shape)
    img = Image.fromarray(viz_events(e,(480,640)))
    img.save(f"vis/{i}.png")
