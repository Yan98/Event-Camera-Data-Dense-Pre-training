...
def eventsToXYTP(events, process=False):
    event_x = events[:, 0].astype(np.int64)
    event_y = events[:, 1].astype(np.int64)
    event_pols = events[:, 3].astype(np.int64)
...
def eventsToVoxel(events, num_bins=5, height=None, width=None, event_polarity=False, temporal_bilinear=True, npy = True):
    event = eventsToVoxelTorch(events, num_bins, height, width, event_polarity, temporal_bilinear)
    if npy:
        event=event.numpy()
    return event
