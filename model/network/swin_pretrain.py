...
class SWIN(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, depths=[2, 2, 6, 2],
                 ...
                 out_indices=(3,), use_checkpoint=False, use_conv = False, init_all = False, mask_dim=None, **kwargs):
        super().__init__()
        print(kwargs)
        ...
        self.fpns = nn.ModuleList([
               ...
               nn.Sequential(
                   nn.AvgPool2d(2),
                   nn.Conv2d(embed_dim*2, embed_dim*8,1,bias=False), 
                   ),
               nn.Sequential(
                   nn.Conv2d(embed_dim*4, embed_dim*8,1,bias=False), 
               ),
               nn.Sequential(
                   nn.ConvTranspose2d(embed_dim * 8, embed_dim * 8, kernel_size=2, stride=2,bias=False),
               ),
               ])
    ...
    def forward_features_list(self,x_list, masks_list=None, masks=None):
        if not isinstance(x_list, list):
            return self.forward_features(x_list, masks)
        output = []
        masks_list = masks if masks_list is None else masks_list
    ...
    def forward_features(self, x,masks=None):
        ...
        if self.ape:
            ...
            absolute_pos_embed = absolute_pos_embed.flatten(2).transpose(1, 2)  # B Wh*Ww C
        else:
